...
class HttpService {
  ...
  Future<Map<int, List<Product>>?> getProductAisleMap() async {
    ...
    if (res.statusCode != 200) {
      print("unable to fetch products from the database, responded with code ${res.statusCode}");
      return null;
    }
    ...
    body.forEach((dynamic data) {
        ...
        if(productAisleMap[_productInfo.aisle] == null) {
          productAisleMap[_productInfo.aisle] = List<Product>.empty(growable: true);
        }
  ...
  Future<Map<String, int>?> getBeaconAisleMap() async {
    ...
    if (res.statusCode != 200) {
      print("unable to fetch mapping from the database, responded with code ${res.statusCode}");
      return null;
    }
  ...
  List<Product> getNearestProducts(Map<String, List<Product>>? productAisleMap) {
    ...
    if(this.nearestBeacons == []) {
      ...
      return _getAllProducts(productAisleMap);
    }
  ...
  List<Product> _getAllProducts(productAisleMap) {
    if(productAisleMap == null) {
      return [];
    }
